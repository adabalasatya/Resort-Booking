...
const UserInvoice = ({ userId }) => {
  ...
  const invoiceHandler = async () => {
    try {
      const response = await fetch(`https://resort-booking-hazel.vercel.app/api/users/${userId}`);
  ...
  const deleteBooking = async (bookingId) => {
    ...
    try {
      const response = await fetch(`https://resort-booking-hazel.vercel.app/api/users/${bookingId}`, {
        method: 'DELETE',
      });
